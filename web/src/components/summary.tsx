...
export function Summary() {
  ...
  const undoCompletionMutation = useMutation({
    mutationFn: (goalId: string) => undoGoalCompletion(goalId),
    onSuccess: () => {
                  ...
                  {goals.map((goal) => {
                    ...
                    return (
                      <li key={goal.id} className="flex items-center gap-2">
                        ...
                        <button
                          type="button"
                          className="cursor-pointer text-zinc-500 text-xs underline underline-offset-2"
                          onClick={() => undoCompletionMutation.mutate(goal.id)}
                        >
                          Desfazer
                        </button>
                      </li>
